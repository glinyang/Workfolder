...
class ReadmeForm(forms.ModelForm):
    mytype = forms.CharField(required=False,widget=forms.Select(choices=MYTYPE_CHOICES))
    myapply = forms.CharField(required=False,widget=forms.Select(choices=APPLY_CHOICES))
    number = forms.CharField(required=False,widget=forms.TextInput(attrs={'placeholder': '201'}))
    mydata1 = forms.CharField(required=False,widget=forms.TextInput(attrs={'placeholder': '2015  Apr.21'}))
